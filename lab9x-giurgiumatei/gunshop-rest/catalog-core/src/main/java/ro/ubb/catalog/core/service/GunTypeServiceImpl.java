...
public class GunTypeServiceImpl implements GunTypeService {
    ...
    public GunType saveGunType(GunType gunType) {
        ...
        if (!gunProviderRepository.existsGunProviderByName(gunType.getGunProvider().getName())) {
            throw new GunShopException("No Gun Provider with this ID!");
        }

        System.out.println(gunProviderRepository.findByName(gunType.getGunProvider().getName()));
        gunType.setGunProvider(gunProviderRepository.findByName(gunType.getGunProvider().getName()).get(0));
    ...
    @Override
    public GunType getGunTypeByName(String name) {
        logger.trace("getGunTypeByName - method entered; name = {}", name);
        GunType result = gunTypeRepository.findByName(name).get(0);
        logger.trace("getGunTypeById - method finished; result = {}", result);
        return result;
    }
